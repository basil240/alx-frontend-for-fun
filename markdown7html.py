...
"""
markdown2html.py: Convert Markdown file to HTML file, parsing heading, list, paragraph, bold, and emphasis syntax.
...
def convert_markdown_to_html(input_file, output_file):
    try:
        with open(input_file, 'r') as f:
            ...
            html_text = parse_bold(html_text)
            html_text = parse_emphasis(html_text)
...
def parse_bold(html_text):
    html_text = re.sub(r'\*\*(.*?)\*\*', r'<b>\1</b>', html_text)
    return html_text

def parse_emphasis(html_text):
    html_text = re.sub(r'__(.*?)__', r'<em>\1</em>', html_text)
    return html_text
